...
<script type="text/javascript">
	...
	for ( var i = 0; i < chkObjs.length; i++) {
		...
		parent.document.getElementById("preview_content").value = parent.document.getElementById("preview_content").value + ss;
		if (ss == "") {
			alert("请选择预习题目。");
			return;
		} 
	...
	function confirm(){
			...
			form1.dispatcher_content.value = real_content ;//选中题目长度
			form1.class_id.value = parent.document.getElementById("classId").value;//选中的班级
			form1.target = parent.parent.parent.document.getElementById("frm_home_main").name;
			var a  = parent.document.getElementById("frame2").contentWindow.document.getElementsByName("student");
			var ss = "";
			for(var i = 0 ;i<a.length;i++){ 
				if(a[i].checked){
					ss = ss+a[i].value+",";//单个学生组成一组字符串
				}
			}
			//
			var a  = parent.document.getElementById("frame0").contentWindow.document.getElementsByName("group");
			var group = "";
			for(var i = 0 ;i<a.length;i++){ 
				if(a[i].checked){
					group = group+a[i].value+",";//单个学生组成一组字符串
				}
			}
			if(group != ""){
				group  = group.substr(0,group.length-1);
				form1.student_id.value = ss + "|" + group;
			}else{
				ss = ss.substr(0,ss.length-1);
				form1.student_id.value = ss;
			}
			form1.subject_id.value = parent.document.getElementById("subject_id").value;
			form1.action = "AddErrorProblem.action";
			form1.submit();
	}
...
<body>
	<form id="form1" name="form1" action="ToErrorSelectItems.action"
		method="post">
		<input type="hidden" name="askLog.itemId" id="itemId" />
		<input type="hidden"  name="error_dispatcher.dispatcher_content" id="dispatcher_content"/>
		<input type="hidden"  name="error_dispatcher.class_id" id="class_id"/>
		<input type="hidden" name="error_dispatcher.student_id" id="student_id">
		<input type="hidden" name="group_error.subject_id" id="subject_id">
...
试题种类：  <s:select
						list="#{'':'全部','0':'单选题','1':'多选题','2':'判断题','3':'问答题'}"
						onchange="sub1()" id="questiontype" name="testItem.questiontype"></s:select>
				</s:if>

				试题名称:<input name="testItem.question" type="text" class="text" /> <input
					name="input" type="button" value=" 搜  索" onclick="sub1()"
					class="bt1 fb grey1 ml50" /> 
					<input name="input" type="button"
					value=" 添  加" onclick="chexiao()" class="bt1 fb grey1 ml50" />
						<input name="input" type="button"
					value=" 确  定" onclick="confirm()" class="bt1 fb grey1 ml50" />
			</div>
					...
					<s:iterator value="testItem.data" status="stat">
						<tr>
							<td><input type="checkbox" name="radio1"
								value="<s:property value='itemId'/>" style="zoom:80%" /></td>
							<td colspan="2"><%=i++%></td>
