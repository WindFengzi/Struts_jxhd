...
function deleteStation(stationId){
	if(window.confirm('你确定要删除吗？')){
		form1.action="DeleteEvaluationItem.action";
		form1.itemId.value = stationId;
		form1.submit();
	}
}
...
function addStation(){
	form1.action="ToInsertEvaluationItem.action";
	form1.submit();
}
function updateStation(id){
	form1.itemId.value=id;
	form1.action="EvaluationItemById.action";
	form1.submit();
}
...
<body>
	<form id="form1" name="form1" action="EvaluationItems.action"
		method="post">
		<input type="hidden" name="evaluationItem.itemId" id="itemId" />
		<s:hidden name="evaluationItem.classId"></s:hidden>
		...
		<div class="w800_con ">
			<div class="tit2">
				<h2 class="lh32 pl30">
					<span class="fb f14">评价内容</span> The Evaluation Information List
				</h2>
			</div>
			<div class="form1 " style="padding-left:20px">
				<input name="evaluationItem.content" type="text"
					value="<s:property value='evaluationItem.content'/>" class="text" />
			...
			<div class="table4 table3">
				<table style="width: 100%;border: 1px; border-color:#d7d7d7;" class="all_content">
					...
					<%
						int i = 1;
					%>
					<s:iterator value="evaluationItem.data" status="stat">
						<tr>
							<td colspan="2"><%=i++%></td>
							...
							<td><a href="#"
								...
								onclick="deleteStation(<s:property value='itemId'/>)"
								class="grey">删除</a></td>
						</tr>
					</s:iterator>
				</table>
				<div class="pages mt20 tc">
					当前是第
					<s:property value="evaluationItem.pageNo" />
					页 共
					<s:property value="evaluationItem.pageNum" />
					页 <a
						href="javascript:firstPage(<s:property value="evaluationItem.pageNo"/>,<s:property value="evaluationItem.pageNum"/>);">第一页</a>
					<a
						href="javascript:prevPage(<s:property value="evaluationItem.pageNo"/>,<s:property value="evaluationItem.pageNum"/>);">上一页</a>
					 <a class="blue12_33"
						href="javascript:nextPage(<s:property value="evaluationItem.pageNo"/>,<s:property value="evaluationItem.pageNum"/>);">下一页</a>
					<a class="blue12_33"
						href="javascript:lastPage(<s:property value="evaluationItem.pageNo"/>,<s:property value="evaluationItem.pageNum"/>);">最后一页</a>
					转到 <input id="inputPageNo" type="text" size="1"
						name="evaluationItem.pageNo"> 页 <a class="blue12_33"
						href="javascript:checkInputPageNo1('inputPageNo',<s:property value="evaluationItem.pageNum"/>)">go</a>
				</div>
			</div>
			</div>
	</form>
