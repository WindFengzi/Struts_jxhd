...
<script type="text/javascript">
				...
				for ( var selIndex = 0; selIndex < toList.options.length; selIndex++) {
					...
					if (toList.options[selIndex].selected) {
						...
						for ( var selIndex1 = toList.options.length - 1; selIndex1 >= selIndex; selIndex1--) {
							...
							toList.options[selIndex1 + 1] = new Option(tex1,
									val1);
						}
						toList.options[transferIndex + 1] = new Option(txt,
								toList.options[selIndex].value + "_" + val);
					}
				}
				if (num > 0) {
	...
	function copyToList1(from, to) {
		...
		for (i = 0; i < fromList.options.length; i++) {
			...
			if (current.selected) {
				...
				//alert(val.indexOf("zuw"));
				if (val.indexOf("_") < 0) {
					alert('不能选择组!');
					return;
				}

				toList.options[toList.length] = new Option(txt
						.replace("--", ""), val);
				fromList.options[i] = null;
				i--;
			}
	...
	function allSelect() {
		...
		for ( var i = 0; i < chsen.length; i++) {
			//chsen.options[i].selected = true;默认选中
			val = val + chsen.options[i].value + ";";
		}
		form1.student_id.value = val;
		...
		form1.action = "ErrorGroupSave.action";
		form1.submit();
	}
	var num = 0;
	function addGroup() {
...
<body>
	...
	<input type="hidden" name="group_error.student_id" id="student_id" />
	<input type="hidden" name="group_error.class_id" id="class_id" />
			...
			<div class="m5 tuxing p10">
				<p class="pl50">
					<input name="" type="button" onclick="addGroup()"
						class="bt2 fb grey1 mr15" value=" 增加分组" id="jiazu" /> 
						<input
		...
		<div class="con1 p30">
			<div class=" w560 zefz">
				<div class="centent fl">
					...
					<div>
						<span id="add" class="aniu1"><a
							href="javascript:copyToList('possible','chosen')"><img
								src="img/cs32.jpg" width="80" height="24" />
						</a>
						</span>
				...
				<div class="centent fr">
					...
					<div>
						<span id="remove" class="aniu2"><a
							href="javascript:copyToList1('chosen','possible')"><img
								src="img/cs33.jpg" width="80" height="24" />
						</a>
						</span>
