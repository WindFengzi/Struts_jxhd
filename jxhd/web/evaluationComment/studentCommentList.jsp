...
<script type="text/javascript">
	...
	//选择年级后的事件
		...
		/* 查看被评价学生具体内容 */
		function lookTheCommentStudent(studentToID) { 
			//alert(studentToID);
			form1.studentToid.value = studentToID;
			form1.target = '';
			form1.action = 'LookTheCommentedStudent.action';
			form1.submit();
		}
...
<body>
	<form id="form1" name="form1" action="StudentCommentList.action"
		...
		<input type="hidden" name="t_teacher_comment.studentTo_id" id="studentToid"/>
			...
			<div class="table4 table3">
				<table style="width:100%;border:1px solid #d7d7d7"
					...
					<tr>
						...
						<td >班级</td>
						<s:if test="t_teacher_comment.tag == 3">
							<td  class="bghui fb">被评价学生</td>
						</s:if>
						<s:elseif test="t_teacher_comment.tag == 2">
							<td  class="bghui fb">被评价老师</td>
						</s:elseif>
						<td  class="bghui fb">操作</td>
					</tr>
					<%
						int i = 1;
					%>
						...
						<tr>
							...
							<%-- <td>
							<input type="checkbox" name="radio1" value="<s:property value='id'/>" style="zoom:80%" /></td> --%>
							<td colspan="2"><%=i++%></td>
							...
							<td><a href="javascript:lookTheCommentStudent(<s:property value='studentTo_id'/>)">查看</a></td>
						</tr>
					</s:iterator>
				</table>
