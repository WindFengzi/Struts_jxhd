...
<script>
	function sub1() {
		...
		var url = 'SaveEvaluationStuAjax.action';
		var studentId = "";
		var gradeId = "";
...
<body>
	<form name="form1" method="post" action="EvaluationResults.action"
			...
			<div class="w800_con ">
				...
				<div class="p20 con1">
					...
					<div class="table4 table3">
						<table style="width:100%;border:1px solid #d7d7d7" id="tab" class="all_content" >
							<tr>
								<td width="5%" class="bghui fb">序号</td>
								<td class="bghui fb">学生姓名</td>
							...
							<%
								...
								for (int n = 0; n < list.size(); n++) {
									student = (t_user) list.get(n);
							%>
							<tr>
								<td><%=student.getUserId()%></td>
								<td><%=student.getName()%></td>
								<td><select id="<%=student.getUserId()%>sel">
										<%-- 
   <option value="" > </option>
   <s:iterator value="evaluationGrade.data" status="stat">
   <option value="<s:property value='gradeId'/>" ><s:property value='gradeDesc'/></option>
   </s:iterator>
   --%>
								</select></td>
